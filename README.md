...
一套用于 open-mp/sa-mp 服务端（Pawn 语言）源码文本提取与回写工具。
目标：快速定位需要翻译文本代码，无需手动翻阅源文件, 统一集中修改之后，一键自动替换原来的内容

**好处**：  
- 不用手动翻阅成百上千个文件去找要翻译的句子。  
- 翻译内容有误或需要更新时，直接在 `translate.json` 中查找修改即可，无需反复打开源码文件。  集中统一操作处理，方便快捷  
- 可以轻松配合 AI 进行翻译：将 JSON 中的文本批量喂给 AI，**极大降低 token 消耗**，大幅度提高翻译效率
- 翻译后可通过py脚本轻松自动替换回源码中
